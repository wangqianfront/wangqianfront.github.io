...
public class DefaultColumn extends Column
{
	...
	if (getColumnType().equals(Constant.COLUMN_TYPE_DATE))
	{
	    // value = (new SimpleDateFormat("dd-MM-yyyy")).format(new Date());
	    if (getValueMap().get(Constant.LOAD_ASSISTANT_DATE) != null)
	    {
		value = getValueMap().get(Constant.LOAD_ASSISTANT_DATE);
	    }
	    else
	    {
		value = defaultDate;
	...
	else if (getColumnType().contains(Constant.COLUMN_TYPE_TIMESTAMP))
	{
	    if (getValueMap().get(Constant.LOAD_ASSISTANT_TIMESTAMP) != null)
	    {
		value = getValueMap().get(Constant.LOAD_ASSISTANT_TIMESTAMP);
	    }
	    else
	    {
	...
	else if (getColumnType().contains(Constant.COLUMN_TYPE_NUM) || getColumnType().equals(Constant.COLUMN_TYPE_BIGING))
	{
	    try
	    {
